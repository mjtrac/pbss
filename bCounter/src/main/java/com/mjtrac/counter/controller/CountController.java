...
import com.mjtrac.counter.service.*;
...
public class CountController {
    ...
    public String index(HttpSession httpSession, Model model) {
        ...
        Path resultsFile = Paths.get(voteTally.getReportOutputDir(), "results_report.html");
        boolean resultsExist = Files.exists(resultsFile);
        model.addAttribute("resultsExist", resultsExist);
        if (resultsExist) {
            try {
                var lastModified = Files.getLastModifiedTime(resultsFile).toInstant()
                    .atZone(java.time.ZoneId.systemDefault());
                model.addAttribute("resultsGeneratedAt",
                    lastModified.format(java.time.format.DateTimeFormatter.ofPattern("MMM d, yyyy h:mm a")));
            } catch (Exception e) {
                model.addAttribute("resultsGeneratedAt", "");
            }
        }
    ...
    private void runScanPass(ScanSession session, String username,
        ...
        try {
            while (written < total && !session.stopRequested) {
                ...
                try {
                    com.mjtrac.counter.service.Point2D[] corners = null;
                    if (result.cornersFound) {
                        corners = new com.mjtrac.counter.service.Point2D[]{
                            new com.mjtrac.counter.service.Point2D(result.bboxTLx, result.bboxTLy),
                            new com.mjtrac.counter.service.Point2D(result.bboxTRx, result.bboxTRy),
                            new com.mjtrac.counter.service.Point2D(result.bboxBRx, result.bboxBRy),
                            new com.mjtrac.counter.service.Point2D(result.bboxBLx, result.bboxBLy),
                        };
                    }
                    if (result.errorMessage != null) {
                        ...
                        if (pStatus == com.mjtrac.counter.service.VoteRecordService.PersistStatus.RACE_SKIP) {
                            retryList.add(new Object[]{imagePath, result});
                        } else if (pStatus == com.mjtrac.counter.service.VoteRecordService.PersistStatus.TRUE_DUPLICATE) {
            ...
            if (!retryList.isEmpty()) {
                ...
                for (Object[] retryItem : retryList) {
                    ...
                    try {
                        com.mjtrac.counter.service.Point2D[] rc = null;
                        if (retryResult.cornersFound) {
                            rc = new com.mjtrac.counter.service.Point2D[]{
                                new com.mjtrac.counter.service.Point2D(retryResult.bboxTLx, retryResult.bboxTLy),
                                new com.mjtrac.counter.service.Point2D(retryResult.bboxTRx, retryResult.bboxTRy),
                                new com.mjtrac.counter.service.Point2D(retryResult.bboxBRx, retryResult.bboxBRy),
                                new com.mjtrac.counter.service.Point2D(retryResult.bboxBLx, retryResult.bboxBLy),
                            };
                        }
                        var retryStatus = voteRecord.persist(
                            ...
                            session);
                        if (retryStatus == com.mjtrac.counter.service.VoteRecordService
                                .PersistStatus.SAVED) {
    ...
    // ── Resume after user stop ──────────────────────────────────────────────
    // Add this endpoint alongside the existing POST /stop
    // and POST /resume (milestone).
    // The existing /resume only clears pauseForResults.
    // This new endpoint handles
    // the user-stop case by also clearing stopRequested.

@PostMapping("/resume-scan")
@ResponseBody
public String resumeScan(HttpSession httpSession,
                          @AuthenticationPrincipal UserDetails userDetails) {
    ScanSession session = getSession(httpSession);
    if (session == null || !session.isStarted()) return "no-session";
    if (session.scanning) return "already-scanning";

    // Clear the stop flag so runScanLoop won't immediately break
    session.stopRequested = false;
    session.scanError     = null;

    // Restart the scan loop in the background thread
    final String username = userDetails != null ? userDetails.getUsername() : "(system)";
    session.scanning = true;
    taskExecutor.execute(() -> runScanLoop(session, username));
    ...
    public String finish(HttpSession httpSession,
        ...
        try {
            java.util.List<com.mjtrac.counter.model.BboxReport.ScanResult> empty =
    ...
    public String newElection(HttpSession httpSession,
        ...
        // Reset per-barcode scribble accumulators so the next election doesn't
        // compare against this one's ballots.
