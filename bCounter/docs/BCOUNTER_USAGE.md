...
bCounter is the ballot scanning and vote-counting application in the pbss
...
Passwords aren't set via a startup property — change your own at `/account/password`
once logged in, or have an ADMIN create/reset accounts at `/admin`.
...
**Default credentials:** `admin` / `ChangeMe123!` (seeded by `CounterDataInitializer`
on first run). Change it at `/account/password` after logging in. This is the
same shared account used for bViewer below — bCounter and bViewer are no longer
...
**Default credentials:** same shared account as bCounter above — `admin` /
`ChangeMe123!` by default. A user needs the `VIEWER` (or `ADMIN`) role to log
in here; change your password at `/viewer/account/password`.
...
where bCounter was launched from, i.e. `pbss/bCounter/`).
...
```bash
./run_all.sh --counter-dir /path/to/pbss/bCounter
```
...
# Scribble detection (ink outside indicator zones — arrows, margin notes, etc.)
scanner.scribble-detection=true
scanner.scribble-indicator-pad-in=0.1   # tolerance zone around each indicator (inches)
...
# Log file
logging.file.name=${user.home}/pbss/logs/bCounter.log
