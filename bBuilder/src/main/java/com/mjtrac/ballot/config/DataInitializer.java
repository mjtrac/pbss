...
public class DataInitializer implements ApplicationRunner {
    ...
    private final com.mjtrac.ballot.service.UserService userService;
    /**
     * Self-injected proxy so seedInitialData() below goes through Spring's AOP
     * proxy (and thus its own @Transactional) when called from run() — a plain
     * `this.seedInitialData()` call would bypass the proxy entirely.
     */
    ...
    public DataInitializer(UserRepository userRepo,
                           ...
                           com.mjtrac.ballot.service.UserService userService,
    ...
    public void run(ApplicationArguments args) {
        ...
        //
        // run() itself is deliberately NOT @Transactional (unlike seedInitialData()
        // below): userService.changePassword() runs REQUIRES_NEW so it commits in
        // its own connection independent of any ambient transaction — necessary
        // because System.exit() below kills the JVM before run() could ever return
        // normally, which is when Spring's AOP proxy would otherwise commit an
        // enclosing @Transactional's work. Keeping run() itself transaction-free
        // also avoids a second, still-open connection racing that commit under
        // SQLite's single-writer model (SQLITE_BUSY).
        if ("true".equalsIgnoreCase(env.getProperty(RESET_FLAG))) {
            ...
            userRepo.findByUsername("admin").ifPresentOrElse(admin -> {
                userService.changePassword(admin.getId(), newPassword);
                System.out.println();
                System.out.println("╔══════════════════════════════════════════════════╗");
    ...
    /**
     * Creates the sample jurisdiction/election/admin/etc on first run.
     * Called via the self-injected proxy (see the `self` field) so this
     * method's own @Transactional actually takes effect — a direct
     * `this.seedInitialData()` call from run() would bypass Spring's AOP
     * proxy and silently run without transaction management.
     */
    @Transactional
