...
public class UserService implements UserDetailsService {
    ...
    /**
     * REQUIRES_NEW so this always commits in its own transaction, independent
     * of any ambient one on the caller's side. DataInitializer's admin password
     * reset (-Dreset.admin.password=true) calls System.exit() right after this
     * returns; if this participated in DataInitializer's own @Transactional
     * run(), the commit — which Spring's AOP proxy only performs on normal
     * method return — would never happen before the JVM dies.
     */
