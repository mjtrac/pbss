...
> *Show the bSuite folder in Finder. It contains ElectionSuite.app, LaunchElection.app, bBuilder, bCounter, bViewer, and the docs folder.*

Double-click **LaunchElection.app**. The Election Suite menu appears — a simple list of actions for all three programs. No installation, no configuration files to edit.
...
> *Open DB Browser for SQLite. File → Open → navigate to bSuite/bCounter/counter_results.db.*
...
Every vote in the election is in this database, queryable with standard SQL. The `docs/query_examples.md` file in the bSuite folder contains additional queries breaking results down by precinct, party, and ballot type, plus notes on running the same queries against PostgreSQL for larger deployments.

---

## Closing (23:00)

bSuite is three Spring Boot applications that work together:
