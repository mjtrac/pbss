# pbss Database Query Examples

The ballot counter writes all scan results to a SQLite database (`counter_results.db`
in the `pbss/bCounter/` folder). You can query it directly with any SQL tool.
...
```bash
# Interactive shell
sqlite3 pbss/bCounter/counter_results.db

# Run a file of SQL
sqlite3 pbss/bCounter/counter_results.db < query.sql

# One-liner
sqlite3 -column -header pbss/bCounter/counter_results.db \
