...
/**
 * Modal login screen — same ScannerUser accounts bScanner/blScanner use.
 * Either role (ADMINISTRATOR or OPERATOR) can sign in, matching bScanner's
 * own access rule for the scan screen. Every attempt, successful or not,
 * is written to AuditLogService.
 */
@Component
class LoginDialog {
    ...
    LoginDialog(ScannerUserRepository userRepository, PasswordEncoder passwordEncoder,
    ...
    boolean showAndAuthenticate(Frame owner) {
        JDialog dialog = new JDialog(owner, "pbss Scanner — Sign In", true);
        ...
        form.add(new JLabel("pbss Scanner"), c);
