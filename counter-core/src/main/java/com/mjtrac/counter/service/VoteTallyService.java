...
import com.mjtrac.counter.model.BboxReport.CandidateTally;
 ...
 * OUTPUT FILES (written to reports.output.dir, alongside the other
 * per-count reports — results_report.html, overvote_report.txt,
 * scribble_report.html, ballot_manifest.csv, cvr_export.csv):
...
public class VoteTallyService {
    ...
    @org.springframework.beans.factory.annotation.Value("${reports.output.dir:${user.home}/pbss_data/reports}")
    private String reportOutputDir;

    /** Separate directory for write-in image crops. */
    @org.springframework.beans.factory.annotation.Value("${data.writeins.dir:${user.home}/pbss_data/writeins}")
    ...
    public void processFinalTally(String imageFolder, ScanSession session) {
        ...
        // Write-in report — only written if any write-in indicator was marked
        try {
            writeWriteInReport();
        } catch (Exception ex) {
            log.warn("Could not write write-in report: {}", ex.getMessage(), ex);
        }
    }
    ...
    private void writeVoteSummary(List<ImageContestResult> icResults, ScanSession session) {
        ...
        Path out = Paths.get(reportOutputDir, "vote_summary.yaml");
        try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(out))) {
    ...
    private void writeWriteInReport() {
        List<ResultsQueryService.WriteInRow> rows;
        try {
            rows = resultsQuery.markedWriteIns();
        } catch (Exception ex) {
            log.debug("Write-in report skipped: {}", ex.getMessage());
            return;
        }
        if (rows.isEmpty()) {
            log.info("No marked write-ins — writein_report.html not written");
            return;
        }

        Path   out = Paths.get(reportOutputDir, "writein_report.html");
        String ts  = java.time.LocalDateTime.now().format(
            java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        ...
        try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(out))) {
            pw.println("<!DOCTYPE html><html lang=\"en\"><head>");
            pw.println("<meta charset=\"UTF-8\"/>");
            pw.printf("<title>Write-In Report — %s</title>%n", h(ts));
            pw.println("<style>");
            pw.println("  body  { font-family:Arial,sans-serif; font-size:11pt;");
            pw.println("          max-width:960px; margin:2rem auto; color:#111; }");
            pw.println("  h1    { font-size:16pt; border-bottom:2px solid #1a6a6a;");
            pw.println("          padding-bottom:.4rem; color:#1a6a6a; }");
            pw.println("  .meta { color:#555; font-size:10pt; margin-bottom:1.5rem; }");
            pw.println("  .info { background:#e6f4ec; border:1px solid #1a6a6a;");
            pw.println("          border-radius:5px; padding:.6rem 1rem;");
            pw.println("          margin-bottom:1.5rem; color:#0f4c3a; }");
            pw.println("  table { border-collapse:collapse; width:100%; }");
            pw.println("  th    { text-align:left; background:#1a6a6a; color:#fff;");
            pw.println("          padding:.3rem .6rem; font-size:10pt; }");
            pw.println("  td    { padding:.3rem .6rem; border-bottom:1px solid #ddd;");
            pw.println("          vertical-align:top; }");
            pw.println("  tr:last-child td { border-bottom:2px solid #1a6a6a; }");
            pw.println("  .num  { text-align:right; font-variant-numeric:tabular-nums; }");
            pw.println("  .name { font-family:monospace; font-size:9.5pt; }");
            pw.println("  @media print { a { color:#1a6a6a; } }");
            pw.println("</style></head><body>");
            pw.printf("<h1>&#9998; Write-In Report</h1>%n");
            pw.printf("<p class=\"meta\">Generated: %s</p>%n", h(ts));
            pw.printf("<div class=\"info\"><strong>%d marked write-in vote(s)</strong> "
                + "found. Only write-in slots whose indicator was actually marked are "
                + "listed here — a write-in name with no mark is not a counted vote. "
                + "Crop images of each write-in area are saved under the writeins/ folder.</div>%n",
                rows.size());
            pw.println("<table><tr><th>#</th><th>Contest</th>"
                + "<th>Write-In Slot</th><th>Image Name</th>"
                + "<th>Barcode</th><th>View</th></tr>");
            int rank = 1;
            for (ResultsQueryService.WriteInRow row : rows) {
                pw.printf("  <tr><td class=\"num\">%d</td>%n", rank++);
                pw.printf("  <td>%s</td>%n", h(row.getContest()));
                pw.printf("  <td>%s</td>%n", h(row.getCandidate()));
                pw.printf("  <td class=\"name\">%s</td>%n", h(row.getImageName()));
                pw.printf("  <td class=\"name\">%s</td>%n", h(row.getBarcodeData()));
                pw.printf("  <td><a href=\"http://localhost:8081/viewer/view"
                    + "?id=%d\" target=\"_blank\">&#128269; View</a></td></tr>%n",
            ...
            pw.println("</table></body></html>");
            log.info("Write-in report written ({} marked): {}",
                rows.size(), out.toAbsolutePath());
        } catch (IOException ex) {
            log.warn("Could not write writein_report.html: {}", ex.getMessage());
        }
    }
