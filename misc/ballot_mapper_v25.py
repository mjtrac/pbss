...
def ocr_contest_title_multiline(gray, contest_ul, contest_lr, dpi):
    ...
    for ln in lines:
        ...
        if len(title_lines) >= 2:   # v23: max 2 lines
            break
...
def ask_contest_title(parent, default_text: str, contest_num: int) -> str | None:
    """
    v23: Simple 2-line contest title editor.
    One Text widget (80 chars wide, 2 lines), OK and Cancel.
    No scrollable rows, no per-line delete buttons.
    """
    win = tk.Toplevel(parent)
    ...
    def ok(event=None):
        ...
        # Collapse to single line for contest title
        result[0] = ' '.join(raw.split())
        win.destroy()
    ...
    label_button(bf, 'OK', ok,
                 bg='#003300', fg='#aaffaa',
                 font=('Helvetica', 11, 'bold')).pack(side='right', padx=8)
    label_button(bf, 'Cancel', cancel,
                 bg='#330000', fg='#ffaaaa').pack(side='right', padx=4)

    # Enter key confirms
    win.bind('<Return>', ok)
    win.bind('<KP_Enter>', ok)

    win.geometry('700x130')
    parent.wait_window(win)
    return result[0]
...
# ── Auto-deskew helpers (ported from CornerDetectionService logic) ─────────

def find_border_line_seed(gray, dpi, threshold=192):
    """
    Scan upward from image bottom for a long horizontal dark run in the
    centre third. Returns (seedX, seedY) or None.
    Mirrors CornerDetectionService.findBorderLineSeed.
    """
    h, w = gray.shape
    near_px = int(0.15 * dpi)
    far_px  = int(1.5  * dpi)
    band    = 8
    min_run = 200   # same as CornerDetectionService.MIN_BORDER_RUN_PX
    left_b  = w // 3
    right_b = 2 * w // 3

    for y in range(h - near_px - 1, max(0, h - far_px), -1):
        y0 = max(0, y - band); y1 = min(h-1, y + band)
        best_len = 0; best_l = -1; best_r = -1
        run_len = 0; run_l = -1
        for x in range(left_b, min(right_b+1, w)):
            col_dark = any(gray[by, x] < threshold
            ...
            if col_dark:
                ...
                if run_len > best_len:
                    best_len = run_len; best_l = run_l; best_r = x-1
                run_len = 0; run_l = -1
        if run_len > best_len:
            best_len = run_len; best_l = run_l
            best_r   = min(right_b, w-1)
        if best_len >= min_run:
            seed_x = (best_l + best_r) // 2
            return seed_x, y
    return None


def measure_tilt_angle(gray, dpi, threshold=192):
    """
    Find the bottom border line and measure its tilt angle in radians.
    Returns angle (positive = clockwise tilt) or 0.0 if not found.
    """
    seed = find_border_line_seed(gray, dpi, threshold)
    if seed is None:
        return 0.0
    seed_x, seed_y = seed
    h, w = gray.shape
    ...
    # Follow line left and right
    def follow(sx, sy, direction):
        cur_y = sy; last_x = sx; last_y = sy
        x = sx + direction
        while 0 <= x < w:
            best_y = -1; best_d = 9999
            for y in range(max(0,cur_y-band), min(h,cur_y+band+1)):
                if gray[y,x] < threshold:
                    d = abs(y - cur_y)
                    if d < best_d: best_d = d; best_y = y
            if best_y < 0: break
            cur_y = best_y; last_x = x; last_y = best_y
            x += direction
        return last_x, last_y

    lx, ly = follow(seed_x, seed_y, -1)
    rx, ry = follow(seed_x, seed_y,  1)

    if rx == lx: return 0.0
    angle = math.atan2(ry - ly, rx - lx)
    print(f'Deskew: tilt={math.degrees(angle):.3f}° '
          f'from ({lx},{ly}) to ({rx},{ry})')
    return angle


def deskew_image(pil_img, angle_rad):
    """
    Rotate PIL image by -angle_rad to remove tilt.
    Returns deskewed PIL image.
    """
    if abs(angle_rad) < 0.0001:
        return pil_img   # no meaningful tilt
    angle_deg = -math.degrees(angle_rad)   # negate to correct tilt
    print(f'Deskewing by {angle_deg:.3f}°')
    return pil_img.rotate(angle_deg, resample=Image.BICUBIC,
                           expand=False, fillcolor=(255,255,255))


# ── Barcode-based YAML naming ──────────────────────────────────────────────

def yaml_name_for_barcode(barcode_str: str, page: int = 1) -> str:
    """
    Convert a barcode string like "1|3|1|1|1|1" to the bCounter YAML
    filename pattern: ballot_E_J_P_T_C_P.yaml
    Matches BboxReportLoader.loadForBarcode() prefix logic.
    """
    parts = [p.strip() for p in barcode_str.split('|')]
    if len(parts) >= 5:
        e,j,p,t,combo = parts[0],parts[1],parts[2],parts[3],parts[4]
    elif len(parts) == 4:
        e,j,p,t,combo = parts[0],parts[1],parts[2],parts[3],'1'
    else:
        # Can't parse — use a safe fallback
        safe = re.sub(r'[^a-zA-Z0-9_-]', '_', barcode_str)
        return f'ballot_{safe}_{page}.yaml'
    return f'ballot_{e}_{j}_{p}_{t}_{combo}_{page}.yaml'


# pyzbar: decodes QR codes AND standard 1D barcodes (Code128, Code39, EAN…)
try:
...
def decode_barcode_from_image(gray, zone_ul=None, zone_lr=None):
    """
    v25: Decode QR code OR standard barcode (Code128, Code39, EAN, etc.)
    from a grayscale numpy image.

    Strategy (in order):
    1. pyzbar on the zone crop (if zone given) — handles all 1D and QR
    2. pyzbar on the full image
    3. cv2.QRCodeDetector on zone crop  (QR only, fallback)
    4. cv2.QRCodeDetector on full image (QR only, fallback)
    5. cv2.barcode_BarcodeDetector on full image (1D, fallback)

    Returns (decoded_string, barcode_type) or (None, None).
    """
    from PIL import Image as _PILImage

    h, w = gray.shape if gray is not None else (0, 0)

    def crop_region(ul, lr):
        if ul is None or lr is None or gray is None:
            return None
        x0=max(0,int(ul[0])-10); y0=max(0,int(ul[1])-10)
        x1=min(w,int(lr[0])+10); y1=min(h,int(lr[1])+10)
        return gray[y0:y1, x0:x1] if x1>x0 and y1>y0 else None

    zone_gray = crop_region(zone_ul, zone_lr)

    # ── 1 & 2: pyzbar (best: handles all formats) ────────────────────────
    if PYZBAR_AVAILABLE and gray is not None:
        for region in ([zone_gray] if zone_gray is not None else []) + [gray]:
            try:
                results = _pyzbar.decode(_PILImage.fromarray(region))
                if results:
                    best = results[0]
                    return best.data.decode('utf-8','replace').strip(), best.type
            except Exception:
                pass

    # ── 3 & 4: cv2 QRCodeDetector fallback ──────────────────────────────
    if CV2_AVAILABLE and gray is not None:
        qr = cv2.QRCodeDetector()
        for region in ([zone_gray] if zone_gray is not None else []) + [gray]:
            try:
                data, bbox, _ = qr.detectAndDecode(region)
                if data:
                    return data.strip(), 'QRCODE'
            except Exception:
                pass

    # ── 5: cv2 BarcodeDetector (1D barcodes) ────────────────────────────
    if CV2_AVAILABLE and hasattr(cv2, 'barcode_BarcodeDetector') and gray is not None:
        try:
            bd = cv2.barcode_BarcodeDetector()
            ok, decoded, types, _ = bd.detectAndDecodeWithType(gray)
            if ok and decoded:
                return decoded[0].strip(), types[0] if types else 'BARCODE'
        except Exception:
            pass
...
class BallotMapper:
    def __init__(self, root, image_path, dpi, out_path, barcode=None, page=1):
        self.root     = root
        self.dpi      = dpi
        self.out_path = out_path

        # v24: store these before any decode/deskew that references them
        ...
        # v24: auto-deskew before displaying
        if CV2_AVAILABLE:
            raw_rgb  = np.array(raw_pil)
            raw_gray = cv2.cvtColor(raw_rgb, cv2.COLOR_RGB2GRAY)
            tilt_rad = measure_tilt_angle(raw_gray, dpi or 300)
            if abs(tilt_rad) > 0.0001:
                self.pil_img = deskew_image(raw_pil, tilt_rad)
            else:
                self.pil_img = raw_pil
        ...
        # v24: attempt barcode decode from full image at startup
        if self.gray is not None and self._barcode is None:
            _decoded, _btype = decode_barcode_from_image(self.gray)
            if _decoded:
                self._barcode = _decoded
                # Only auto-name if out_path is still the default
                if self.out_path in ('ballot_template.yaml', None):
                    self.out_path = yaml_name_for_barcode(_decoded, self._page)
                print(f'Auto-decoded {_btype}: {_decoded!r} → {self.out_path}')
        ...
        # (img_path, _barcode, _page already set above)
        self.columns  = []   # [{'ul','lr'}]
    ...
    def _on_release(self,event):
        ...
        if phase=='layout':
            ...
            # v24: try to decode QR/barcode from the dragged zone
            if any(kw in lbl.lower() for kw in ('qr','barcode','bar code')):
                decoded, btype = decode_barcode_from_image(
                    self.gray, ul, lr) if self.gray is not None else (None, None)
                if decoded:
                    self._barcode = decoded
                    new_name = yaml_name_for_barcode(decoded, self._page)
                    self.out_path = new_name
                    self.status_var.set(
                        f'Decoded {btype}: {decoded!r} → output: {new_name}  (hover+x to delete zone)')
                else:
                    # Try full image
                    decoded, btype = decode_barcode_from_image(self.gray) \
                                     if self.gray is not None else (None, None)
                    if decoded:
                        self._barcode = decoded
                        new_name = yaml_name_for_barcode(decoded, self._page)
                        self.out_path = new_name
                        self.status_var.set(
                            f'Decoded {btype} from full image: {decoded!r} → output: {new_name}')
                    else:
                        self.status_var.set(
                            f'Added zone: {lbl!r} — could not decode barcode')
            else:
                self.status_var.set(f'Added layout zone: {lbl!r}  (hover+x to delete)')
            self._render()
            ...
            if self._confirm(desc):
                ...
                self.status_var.set(f'Added marker. Hover+x to delete.')
                self._render()
            ...
            if self._confirm(desc):
                ...
                self._snap_lines=[]
                self.status_var.set(f'Added column {len(self.columns)}. Hover+x to delete.')
                self._render()
    ...
    def _on_canvas_motion(self, event):
        ...
        # Check contests (centre of bbox)
        for i, ct in enumerate(self.contests):
            ctx = (ct['ul'][0]+ct['lr'][0])/2
            cty = (ct['ul'][1]+ct['lr'][1])/2
            d = math.hypot(ix-ctx, iy-cty)
            if d < best_dist:
                best_dist = d; best_item = ('contest', i)
    ...
    def _on_canvas_nudge(self, event):
        ...
        k = event.keysym
        if k == 'x':
            if result['matches'] and idx is not None and idx < len(result['matches']):
        ...
        if k == 'x':
            if kind=='marker' and idx < len(self.markers):
                ...
                self.status_var.set(f'Deleted marker {idx+1}')
                self._render()
            elif kind=='zone' and idx < len(self.layout_zones):
                self.layout_zones.pop(idx)
                self._hovered_item = None
                self.status_var.set(f'Deleted layout zone {idx+1}')
                self._render()
            elif kind=='column' and idx < len(self.columns):
                self.columns.pop(idx)
                self._hovered_item = None
                self.status_var.set(f'Deleted column {idx+1}')
                self._render()
            elif kind=='contest' and idx < len(self.contests):
                self.contests.pop(idx)
                self._hovered_item = None
                self.status_var.set(f'Deleted contest {idx+1}')
                self._render()
            return
    ...
    def _generate_yaml(self):
        ...
        L = [
            ...
            f'# Barcode: {self._barcode}' if self._barcode else
            '# Barcode: (not specified -- use --barcode for bCounter naming)',
...
def main():
    p=argparse.ArgumentParser(description=f'Ballot Mapper v{VERSION}')
    p.add_argument('image')
    p.add_argument('--out', default=None,
                   help='Output YAML filename. If omitted and --barcode '
                        'is given, auto-named ballot_E_J_P_T_C_1.yaml')
    p.add_argument('--dpi', type=int, default=None)
    p.add_argument('--barcode', default=None,
                   help='Override barcode/QR string. Normally auto-decoded '
                        'from image (QR, Code128, Code39, EAN, etc.). '
                        'Use only if auto-decode fails.')
    p.add_argument('--page', type=int, default=1,
                   help='Page number for multi-page ballots (default 1)')
    args=p.parse_args()
    ...
    if args.out:
        ...
        out_path = yaml_name_for_barcode(args.barcode, args.page)
        print(f'Output YAML: {out_path}  (from barcode {args.barcode!r})')
    else:
        out_path = 'ballot_template.yaml'
    dpi=args.dpi
    ...
    BallotMapper(root,args.image,dpi,out_path,
                 barcode=args.barcode, page=args.page)
    root.mainloop()
