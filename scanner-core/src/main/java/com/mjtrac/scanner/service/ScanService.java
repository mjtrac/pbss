...
public class ScanService {
    ...
    public synchronized void startScan(String comment) {
        ...
        // "Picked up when the scan starts or restarts": logged immediately,
        // with its own timestamp, rather than waiting for the batch-summary
        // log entry writeBatchLog() writes at completion.
        if (!session.comment.isBlank()) {
            appendNoteLog("Start note", session.comment, null);
        }
    ...
    /**
     * Records an end note against the most recently completed batch —
     * e.g. to flag a misfeed/doublefeed for manual correction. Logged
     * immediately with its own timestamp (not tied to the batch's own
     * completion time, since an operator may write this well after the
     * scan actually finished).
     */
    public synchronized void saveEndNote(String note) {
        if (note == null || note.isBlank()) return;
        appendNoteLog("End note", note, session.lastBatchId);
    }
    ...
    /**
     * Appends a timestamped note entry to batch_log.txt immediately (not
     * deferred to batch completion — see startScan()/saveEndNote()), then
     * optionally prints it as a flag page (see FlagPagePrinter — off by
     * default, and a printing failure here must not affect the log write,
     * which already happened, or the calling scan operation).
     */
    private void appendNoteLog(String label, String note, String batchId) {
        String timestamp = LocalDateTime.now().format(LOG_TS);
        try {
            String logDir = (config.batchLogDir != null && !config.batchLogDir.isBlank())
                ? config.batchLogDir
            ...
            try (PrintWriter pw = new PrintWriter(new FileWriter(logFile.toFile(), true))) {
                pw.printf("%n── %s @ %s%s ──%n", label, timestamp,
                    batchId != null ? " (batch " + batchId + ")" : "");
                pw.println(note);
            }
            log.info("{} logged: {}", label, note);
        } catch (IOException e) {
            log.warn("Could not write {}: {}", label.toLowerCase(), e.getMessage());
        }

        FlagPagePrinter.printIfEnabled(config, label, timestamp, note);
    }
