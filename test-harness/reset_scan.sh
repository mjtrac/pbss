...
# Run from any directory; paths are relative to pbss root.
#
# Usage:
...
IMAGES_DIR="${HOME}/pbss_data/cast_ballot_scans"  # default
...
if [[ -z "$DB_DIR" ]]; then
  ...
  echo "     Falling back to \$HOME/pbss_data/db"
  DB_DIR="${HOME}/pbss_data/db"
fi

echo "── Reset bCounter scan state ─────────────────────────────────"
echo "   pbss:  $BSUITE_DIR"
echo "   DB dir:  $DB_DIR"
...
for dir in "${HOME}/pbss_data/writeins" "${HOME}/pbss_data/scribbles" "${HOME}/pbss_data/reports"; do
    if [ -d "$dir" ]; then
