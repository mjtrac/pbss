...
def parse_args():
    ...
    p.add_argument("--images",   default=str(Path.home() / "pbss_data/cast_ballot_scans"),
    ...
    p.add_argument("--new-election",  action="store_true",
                   help="Clear ALL persisted vote data via POST /new-election "
                        "before scanning, not just the in-memory session")
    return p.parse_args()
...
class BCounterClient:
    ...
    def new_election(self):
        """Clear all persisted vote data (DB) — not just the in-memory session."""
        csrf = self._csrf()
        r = self.session.post(f"{self.host}/new-election", data={"_csrf": csrf},
                              allow_redirects=True)
        r.raise_for_status()
...
def main():
    ...
    if args.new_election:
        print("  Clearing all persisted vote data (POST /new-election)...")
        client.new_election()
