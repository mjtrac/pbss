...
Automated test harness for the full bSuite pipeline:
...
**Running apps:**
- bBuilder at `http://localhost:8080` (start via LaunchElection.app or `./mvnw spring-boot:run` in bBuilder/)
...
```bash
rm ~/bSuite/bCounter/counter_results.db
```
...
python3 verify_results.py --db ~/bSuite/bCounter/counter_results.db
